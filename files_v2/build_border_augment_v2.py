...
def main():
    ...
    def wet_candidates(c, yr, include_oos):
        out = [f for f in ar_fips if f != c and wet.get((f, yr), 0) == 1]
        if include_oos:
    ...
    cohort = {r['fips']: int(r['cohort']) for r in rows if r.get('treated_unit')=='1' and r.get('cohort') not in (None,'','0')}
